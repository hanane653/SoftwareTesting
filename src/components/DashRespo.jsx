  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setUsers(usersRes.data || mockUsers); // Utilisez les données mockées si l'API n'existe pas
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full divide-y divide-gray-200">
        ...
        <tr>
                      ...
                      <th className='px-6 py-6'>Livrables</th>
          ...
          {demandes.map((demande, index) => (
            <tr key={demande.id} className="hover:bg-gray-50">
              <td className="px-6 border-r py-2">{demande.nomProjet}</td>
                        ...
                        <td>
                          <button className="bg-orange-600 text-white px-4 py-1 rounded hover:bg-orange-700">Charger livrables</button>
                        </td>
            </tr>
          ))}
        </tbody>
